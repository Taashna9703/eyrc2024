...
# Team ID:          	[ 1391 ]
# Author List:		[  Jay Patel, Vidhi Pathak,Hiteshree V Varsale,Shiv Patel ]
...
def detect_aruco(image):
    ...
    aruco_dict = cv2.aruco.PredefinedDictionary(cv2.aruco.DICT_4X4_50)
    ...
    for i in range(len(ids)):
        ...
        if area > aruco_area_threshold:
            rv, tv, _ = cv2.aruco.estimatePoseSingleMarkers(corners, size_of_aruco_m, cam_mat, dist_mat)
            rvec = rv[i][0]
            tvec = tv[i][0]

            aruco_center = (int((x1 + x2 + x3) / 3), int((y1 + y2 + y3) / 3))
            xc, yc = map(int, aruco_center)

            center_aruco_list.append(aruco_center)
            depth_value = image[yc, xc]
            distance_from_rgb = depth_value / 1000
            distance_from_rgb_list.append(distance_from_rgb)

            aruco_list = rvec[0].tolist()
            angle_aruco = math.degrees(math.atan2(aruco_list[0], aruco_list[2]))
            angle_aruco = (0.788*angle_aruco) - ((angle_aruco**2)/3160)
            angle_aruco_list.append(angle_aruco)

            width_aruco_list.append(width)
            cv2.aruco.drawDetectedMarkers(image, corners, ids)
            cv2.putText(image, f"ID: {ids[i]}", (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2, cv2.LINE_AA)
            cv2.drawFrameAxes(image, cam_mat, dist_mat, rvec, tvec, size_of_aruco_m)
            cv2.circle(image, aruco_center, 5, (0, 0, 255), -1)
...
class aruco_tf(Node):
    ...
    def depthimagecb(self, data):
        ...
        bridge = CvBridge()
        self.depth_image = bridge.imgmsg_to_cv2(data, data.encoding)
    ...
    def colorimagecb(self, data):
        ...
        bridge = CvBridge()
        self.cv_image = bridge.imgmsg_to_cv2(data, data.encoding)
    ...
    def process_image(self):
        ...
        ############ ADD YOUR CODE HERE ############
        center_list, distance_list, angle_list, width_list, ids, _ = detect_aruco(self.cv_image)
        rpy = []
        list_rpy = []
        ...
        tranform_matrix = np.zeros([0, 0, 1], [-1, 0, 0], [0, 1,0])

        mat = np.dot(rot_mat, tranform_matrix)
        egod = tf_transformations.euler_from_matrix(egod)
        ...
        q_rot = tf_transformations.quaternion_from_euler(roll, pitch, yaw)
        ...
        y_1 = dist/1000 * (sizeCamX - center_list[i][0] - centerCamX) / focalX
        z_1 = dist/1000 * (sizeCamY - center_list[i][1] - centerCamY) / focalY
        x_1 = dist/1000

        tran_msg = TransformStamped()
        tran_msg.header.stamp = self.get_clock().now().to_msg()
        tran_msg.header.frame_id = 'camera_link'
        tran_msg.child_frame_id = f'cam_{aruco_id}'
        tran_msg.transform.translation.x = x_1
        tran_msg.transform.translation.y = y_1
        tran_msg.transform.translation.z = z_1
        tran_msg.transform.rotation.x = qx
        tran_msg.transform.rotation.y = qy
        tran_msg.transform.rotation.z = qz
        tran_msg.transform.rotation.w = qw
        self.br.sendTransform(tran_msg)
        
        t = self.tf_buffer.lookup_transform('base_link', f'cam_{aruco_id}', rclpy.time.Time())
        tran_msg.header.stamp = self.get_clock().now().to_msg()
        tran_msg.header.frame_id = 'base_link'
        tran_msg.child_frame_id = f'obj_{aruco_id}'
        tran_msg.transform.translation.x = t.transform.translation.x
        tran_msg.transform.translation.y = t.transform.translation.y
        tran_msg.transform.translation.z = t.transform.translation.z
        tran_msg.transform.rotation.x = t.transform.rotation.x
        tran_msg.transform.rotation.y = t.transform.rotation.y
        tran_msg.transform.rotation.z = t.transform.rotation.z
        tran_msg.transform.rotation.w = t.transform.rotation.w
        self.br.sendTransform(tran_msg)
